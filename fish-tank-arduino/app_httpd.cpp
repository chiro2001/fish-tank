...
static esp_err_t stream_handler(httpd_req_t *req) {
  ...
  while (true) {
    ...
    Serial.printf(
        "MJPG: %uB %ums (%.1ffps), AVG: %ums (%.1ffps), %u+%u+%u+%u=%u %s%d\n",
...
const char feed_html[] =
    "<html><head><title>Feed "
    "Result</title></head><body><h3>OK.</h3></body></html>";

static esp_err_t feed_handler(httpd_req_t *req) {
  httpd_resp_set_type(req, "text/html");
  extern bool feed_run;
  feed_run = true;
  return httpd_resp_send(req, feed_html, sizeof(feed_html) / sizeof(char));
}
